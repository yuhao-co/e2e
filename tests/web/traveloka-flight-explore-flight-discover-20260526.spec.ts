...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Discover.
 *   Navigates to the flight-discover surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-ohka-components, fpr-ohka
 * EN Domain: flight-discover
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-05-26T04:00:55.492Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/discover';
...
test('Traveloka flight Flight Discover smoke coverage (20260526)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Discover audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-discover: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
