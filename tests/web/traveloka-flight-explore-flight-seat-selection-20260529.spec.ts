...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Seat Selection.
 ...
 *   Navigates to the flight-seat-selection surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-post-seat-components, fpr-post-seat
 * EN Domain: flight-seat-selection
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection, sub-flow-redirect
 * EN Generated: 2026-05-29T05:45:39.999Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/seat-selection/TEST_BOOKING_ID';
...
test('Traveloka flight Flight Seat Selection smoke coverage (20260529)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Seat Selection audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-seat-selection: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
