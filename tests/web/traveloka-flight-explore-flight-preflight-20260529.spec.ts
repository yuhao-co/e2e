...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Pre-flight Check.
 ...
 *   Navigates to the flight-preflight surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-preflight-mobile, fpr-preflight
 * EN Domain: flight-preflight
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection, sub-flow-redirect
 * EN Generated: 2026-05-29T05:45:40.000Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/preflight/landingPage/TEST_BOOKING/TEST_ITINERARY';
...
test('Traveloka flight Flight Pre-flight Check smoke coverage (20260529)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Pre-flight Check audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-preflight: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
