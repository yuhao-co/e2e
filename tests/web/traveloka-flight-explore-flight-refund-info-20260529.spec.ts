...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Refund Info.
 ...
 *   Navigates to the flight-refund-info surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-refund-communication, fpr-refund-components, fpr-refund
 * EN Domain: flight-refund-info
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection, sub-flow-redirect
 * EN Generated: 2026-05-29T04:00:06.891Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/refund/info/TEST_BOOKING_ID';
...
test('Traveloka flight Flight Refund Info smoke coverage (20260529)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Refund Info audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-refund-info: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
