...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Group Booking.
 *   Navigates to the flight-group-booking surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-group-booking-components, fpr-group-booking
 * EN Domain: flight-group-booking
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-05-27T04:00:02.334Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/group-booking';
...
test('Traveloka flight Flight Group Booking smoke coverage (20260527)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Group Booking audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-group-booking: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
