...
/**
 * EN Purpose: Auto-generated smoke spec for Visa Info.
 *   Navigates to the flight-visa-info surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-visa-components, fpr-visa
 * EN Domain: flight-visa-info
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-06-04T04:00:05.877Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/visa/visa-info';
...
test('Traveloka flight Visa Info smoke coverage (20260604)', async ({ page }) => {
  ...
  console.log(`\n📋 Visa Info audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-visa-info: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
