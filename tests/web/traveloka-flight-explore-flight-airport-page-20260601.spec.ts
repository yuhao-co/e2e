...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Airport Page.
 *   Navigates to the flight-airport-page surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-airport
 * EN Domain: flight-airport-page
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-06-01T04:00:07.442Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/airport/cgk';
...
test('Traveloka flight Flight Airport Page smoke coverage (20260601)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Airport Page audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-airport-page: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
