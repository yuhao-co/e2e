...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Reschedule Info.
 ...
 *   Navigates to the flight-reschedule-info surface and runs P0 generic bug detection.
 ...
 * EN Domain: flight-reschedule-info
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection, sub-flow-redirect
 * EN Generated: 2026-05-28T04:00:03.978Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/reschedule/info/TEST_BOOKING_ID';
...
test('Traveloka flight Flight Reschedule Info smoke coverage (20260528)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Reschedule Info audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-reschedule-info: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
