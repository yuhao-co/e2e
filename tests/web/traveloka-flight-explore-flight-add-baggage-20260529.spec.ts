...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Add Baggage.
 ...
 *   Navigates to the flight-add-baggage surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-extra-baggages-common, fpr-extra-baggages-mobile-components, fpr-extra-baggages-mobile, fpr-extra-baggages
 * EN Domain: flight-add-baggage
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection, sub-flow-redirect
 * EN Generated: 2026-05-29T04:00:06.894Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/add-baggage/TEST_BOOKING_ID';
...
test('Traveloka flight Flight Add Baggage smoke coverage (20260529)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Add Baggage audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-add-baggage: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
