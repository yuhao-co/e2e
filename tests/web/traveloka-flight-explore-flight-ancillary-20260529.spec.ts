...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Ancillary Add-ons.
 ...
 *   Navigates to the flight-ancillary surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-ancillary-components, fpr-ancillary
 * EN Domain: flight-ancillary
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection, sub-flow-redirect
 * EN Generated: 2026-05-29T05:45:39.998Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/ancillary/TEST_SPEC';
...
test('Traveloka flight Flight Ancillary Add-ons smoke coverage (20260529)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Ancillary Add-ons audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-ancillary: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
