...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Changes.
 *   Navigates to the flight-changes surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-flight-changes
 * EN Domain: flight-changes
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-06-03T04:00:06.395Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/changes';
...
test('Traveloka flight Flight Changes smoke coverage (20260603)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Changes audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-changes: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
