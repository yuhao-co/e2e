...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Cheapest Date.
 *   Navigates to the flight-cheapest-date surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-cheapest-date-travel
 * EN Domain: flight-cheapest-date
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-05-27T04:00:02.333Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/cheapestDate';
...
test('Traveloka flight Flight Cheapest Date smoke coverage (20260527)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Cheapest Date audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-cheapest-date: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
