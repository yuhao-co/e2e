...
/**
 * EN Purpose: Auto-generated smoke spec for Flight Country Page.
 *   Navigates to the flight-country-page surface and runs P0 generic bug detection.
 *   Promoted from candidate → stable once it passes consistently for 3+ weeks.
 * EN Source: explore-coverage-gaps.ts — gap scan of www packages: fpr-country-page-components, fpr-country-page
 * EN Domain: flight-country-page
 * EN Lifecycle: candidate
 * EN Surface: desktop
 * EN Concerns: page-load, p0-detection
 * EN Generated: 2026-05-29T06:18:15.202Z
 */

const TARGET_URL = 'https://www.traveloka.com/en-sg/flight/country/indonesia';
...
test('Traveloka flight Flight Country Page smoke coverage (20260529)', async ({ page }) => {
  ...
  console.log(`\n📋 Flight Country Page audit: ${auditResult.length} total issues, ${p0Issues.length} P0`);
  if (p0Issues.length > 0) {
    console.error('P0 issues found:', JSON.stringify(p0Issues, null, 2));
  }

  expect(p0Issues, `P0 bugs found on flight-country-page: ${JSON.stringify(p0Issues)}`).toHaveLength(0);
});
