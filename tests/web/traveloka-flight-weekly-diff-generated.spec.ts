...
const sourceContext = {
  ...
  doc: 'Metasearch_Q2-26_Improvement.md',
  requirement: {
    title: 'Email Confirmation field',
    why: 'JP metasearch partner sees repeated complaints about users not receiving email; two email fields reduce input mistakes.',
    risk: 'Domestic routes cannot be enabled for Traveloka if this metasearch requirement is not enabled.',
    scope: ['config per affiliateId in BE', 'AB test in BE', 'direct metasearch only'],
  },
  changedFiles: [
...
/**
 * EN Purpose: Validate the metasearch booking contact flow for the email confirmation feature introduced by PR 33383, using the direct-metasearch requirement from Metasearch_Q2-26_Improvement.md.
 * 中文目的: 基于 PR 33383 与 Metasearch_Q2-26_Improvement.md，验证 metasearch 预订联系信息页新增的邮箱确认能力。
 ...
 * EN Main checks: start from fullsearch results, click choose and select to reach booking; show confirmation field only on eligible metasearch booking flow; block submit when confirmation is empty or mismatched; allow submit path once both email values match.
 * 中文校验项: 从 fullsearch 结果页出发，点击 choose 和 select 进入 booking；仅在符合条件的 metasearch 预订流展示确认字段；确认邮箱为空或不一致时阻止提交；两次邮箱一致后允许继续提交链路。
 * EN Source commits: PR 33383 - [FEATURE][FLIGHT] email confirmation.
 * 中文来源提交: PR 33383 - [FEATURE][FLIGHT] email confirmation。
 * EN Source summary: JP metasearch partners requested email confirmation because users often complain that they do not receive emails; scope is affiliate-configured, AB-tested, and direct-metasearch only.
 * 中文来源摘要: JP metasearch 渠道因用户频繁反馈收不到邮件而提出邮箱确认需求；范围受 affiliate 配置、AB test 与 direct metasearch 条件限制。
